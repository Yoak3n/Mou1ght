...
func setupArticleRouter(r fiber.Router) {
	...
	article.Post("/status", middleware.Auth, handler.UpdatePostStatus)
	article.Post("/list", handler.ListPostPublic).Name("list")
	article.Post("/list/admin", middleware.Auth, handler.ListPost).Name("list_admin")
...
func setupSharingRouter(r fiber.Router) {
	...
	sharing.Post("/list", handler.ListPostPublic).Name("list")
	sharing.Post("/list/admin", middleware.Auth, handler.ListPost).Name("list_admin")
	sharing.Post("/status", middleware.Auth, handler.UpdatePostStatus)
...
func setupMessageRouter(r fiber.Router) {
	...
	message.Get("/visitor", handler.VisitorID)
	...
	message.Get("/owned", handler.OwnedMessageIDs)
	message.Post("/list", handler.ListMessagePublic).Name("list")
	message.Post("/list/admin", middleware.Auth, handler.ListMessage).Name("list_admin")
